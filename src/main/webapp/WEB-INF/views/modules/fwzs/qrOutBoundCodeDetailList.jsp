...
<ul class="nav nav-tabs">
    <c:choose>
        <c:when test ="${receiverType eq 'dealer'}">
            <li class="active"><a href="${ctx}/fwzs/outBound/qrcodeDetailList?id=${outBound.id}">发货单明细</a></li>
        </c:when>
        <c:otherwise>
            <li class="active"><a href="${ctx}/fwzs/outBound/qrcodeDetailList?id=${outBound.id}">出库单明细</a></li>
        </c:otherwise>
    </c:choose>
</ul><br/>

<form:form id="searchForm" modelAttribute="outBound" action="${ctx}/fwzs/outBound/qrcodeDetailList" method="post" class="breadcrumb form-search">
    ...
    <div>
        ...
        <form:input path="outBoundNo" htmlEscape="false" maxlength="50" class="input-medium" readonly="true"/>
