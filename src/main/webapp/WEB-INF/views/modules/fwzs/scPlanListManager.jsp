...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/fwzs/manager/scPlan/">生产管理列表</a></li>
		<%-- <shiro:hasPermission name="fwzs:scPlan:edit"><li><a href="${ctx}/fwzs/manager/scPlan/form">任务计划添加</a></li></shiro:hasPermission> --%>
	</ul>
	<form:form id="searchForm" modelAttribute="scPlan" action="${ctx}/fwzs/manager/scPlan/" method="post" class="breadcrumb form-search">
							...
							<tr>
								...
								<shiro:hasPermission name="fwzs:scPlan:edit"><td rowspan="${listSize}">
									<table class="operationTable">
										<tr>
											<c:if test="${scPlan.status == '3'}">
												<td style="border-style:none;">
													<a href="${ctx}/fwzs/manager/scPlan/form?id=${scPlan.id}">修改</a>
												</td>
											</c:if>

											<c:if test="${scPlan.status == '5' || scPlan.status == '6' || scPlan.status == '7'}">
												<td style="border-style:none;">
													<a href="${ctx}/fwzs/scPlan/review?id=${scPlan.id}&formTitle=dataManagement">查看</a>
												</td>
											</c:if>
										</tr>
									</table>
