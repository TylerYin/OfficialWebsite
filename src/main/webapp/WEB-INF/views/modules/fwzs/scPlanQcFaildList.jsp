...
<body>
	<ul class="nav nav-tabs">
        <li class="active"><a href="${ctx}/fwzs/check/findPlanQcFaildList">质检未通过报告列表</a></li>
	</ul>
    <div id="qcReason" class="hidden"></div>
    <div id="findQcNotPassURL" class="hidden">${ctx}/fwzs/check/scPlan/findQcNotPass</div>
	<form:form id="searchForm" modelAttribute="scPlan" action="${ctx}/fwzs/check/scPlan/findPlanQcFaildList" method="post" class="breadcrumb form-search">
							...
							<c:when test="${status.index eq 0}">
								<tr>
                                    ...
                                    <c:if test="${scPlanType eq '7'}">
                                        <td rowspan="${listSize}" title="${scPlan.qcNotPassReason}">
                                            ${scPlan.qcCount}
                                        </td>
                                    </c:if>

                                    <td rowspan="${listSize}" class="showQcNotPassReason">
                                        ${scPlan.qcCount}<span class="hidden">${scPlan.id}</span>
                                    </td>
									...
									<td rowspan="${listSize}">
										<shiro:hasPermission name="fwzs:scPlan:edit">
                                            <a href="${ctx}/fwzs/check/scPlan/qcFaildForm?id=${scPlan.id}">修改</a>
										</shiro:hasPermission>
									</td>
								</tr>
							</c:when>
