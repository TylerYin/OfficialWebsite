...
<body>
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/fwzs/warehouse">仓库列表</a></li>
		<shiro:hasPermission name="fwzs:dealer:edit"><li><a href="${ctx}/fwzs/warehouse/form">仓库添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="warehouse" action="${ctx}/fwzs/warehouse/" method="post" class="breadcrumb form-search">
		<c:set var="warehouseModel" value="${warehouse}"/>
		...
		<ul class="ul-form">
			...
			<li><label>区域：</label>
				<c:choose>
					<c:when test="${not empty warehouse.salesArea.name}">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="warehouse">
			<tr>
				<td>
					<a href="${ctx}/fwzs/warehouse/form?id=${warehouse.id}">
                ...
                <td>
                    <c:if test="${not empty warehouse.parentWarehouse}">
				...
				<td>
					<c:if test="${not empty warehouse.company}">
						${warehouse.company.name}
					</c:if>
				</td>
				<td>
					<c:if test="${not empty warehouse.salesArea}">
						${warehouse.salesArea.name}
					</c:if>
				</td>
                <td>
                    ${warehouse.volume}
				...
				<td>
                    <c:if test="${not empty warehouse}" >
                        <c:choose>
                            ...
                            <c:when test="${warehouse.grade eq '2'}">
                                二级
                            </c:when>
                            <c:otherwise>
                                三级
				...
				<shiro:hasPermission name="fwzs:dealer:edit"><td>
					<a href="${ctx}/fwzs/warehouse/form?id=${warehouse.id}">修改</a>
					<a href="javascript:WarehouseList.Events.delete('${warehouse.id},${warehouseModel.name},${warehouseModel.phone},${warehouseModel.salesArea.id}, ${warehouseModel.salesArea.name},${warehouseModel.company.name}');" onclick="return confirmx('确认要删除该仓库吗？', this.href)">删除</a>
	...
	<script src="${ctxStatic}/customized/warehouseList.js"></script>
