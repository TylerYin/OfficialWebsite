...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/fwzs/check/scPlan/findPlanQcFaildList">检验报告未通过列表</a></li>
		<li class="active"><a href="${ctx}/fwzs/check/scPlan/qcFaildForm?id=${scPlan.id}">生产计划<shiro:hasPermission name="fwzs:scPlan:edit">${not empty scPlan.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="fwzs:scPlan:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	...
	<form:form id="inputForm" modelAttribute="scPlan" action="${ctx}/fwzs/check/scPlan/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<input name="editQcNotPassPlan" type="hidden" value="editQcNotPassPlan"/>
        ...
        <div class="control-group">
            <c:if test="${scPlan.status ne '6'}">
                ...
                <div class="controls">
                    <form:textarea path="qcNotPassReason" readonly="true" htmlEscape="false" maxlength="166" class="input-xlarge "/>
                </div>
            </c:if>
        </div>
					...
					<c:if test="${not empty scPlan.bsProductList}">
						<c:forEach items="${scPlan.bsProductList}" var="bsProduct" varStatus="status">
							<tr>
								...
								<td>
									<div class="control-group">
										<form:input id="planNumber" path="bsProductList[${status.index}].planNumber" htmlEscape="false" maxlength="9" class="input-xlarge inputWidth115 required"/>
									</div>
								</td>
