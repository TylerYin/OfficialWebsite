		...
		<c:forEach items="${page.list}" var="link">
			<tr>
				...
				<shiro:hasPermission name="cms:link:edit"><td>
                    <c:choose>
                        <c:when test="${link.delFlag eq '0'}">
                            <a href="${ctx}/cms/link/delete?id=${link.id}&status=${link.delFlag}" onclick="return confirmx('确认要删除该链接吗？', this.href)" >删除</a>
                        </c:when>
                        <c:when test="${link.delFlag eq '1'}">
                            <a href="${ctx}/cms/link/delete?id=${link.id}&status=${link.delFlag}" onclick="return confirmx('确认要恢复审核该链接吗？', this.href)" >恢复审核</a>
                        </c:when>
                        <c:otherwise>
                            <a href="${ctx}/cms/link/delete?id=${link.id}&status=${link.delFlag}" onclick="return confirmx('确认要发布该链接吗？', this.href)" >发布</a>
                        </c:otherwise>
                    </c:choose>
