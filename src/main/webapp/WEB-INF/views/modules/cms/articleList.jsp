	    				...
	    				<shiro:hasPermission name="cms:article:audit">
                            <c:choose>
                                <c:when test="${article.delFlag eq '0'}">
                                    <a href="${ctx}/cms/article/delete?id=${article.id}&status=${article.delFlag}&categoryId=${article.category.id}" onclick="return confirmx('确认要删除该文章吗？', this.href)" >删除</a>
                                </c:when>
                                <c:when test="${article.delFlag eq '1'}">
                                    <a href="${ctx}/cms/article/delete?id=${article.id}&status=${article.delFlag}&categoryId=${article.category.id}" onclick="return confirmx('确认要恢复审核该文章吗？', this.href)" >恢复审核</a>
                                </c:when>
                                <c:otherwise>
                                    <a href="${ctx}/cms/article/delete?id=${article.id}&status=${article.delFlag}&categoryId=${article.category.id}" onclick="return confirmx('确认要发布该文章吗？', this.href)" >审核通过</a>
                                </c:otherwise>
                            </c:choose>
						</shiro:hasPermission>
					</shiro:hasPermission>
