...
<head>
	...
	<script type="text/javascript">
		...
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
				...
				data:{simpleData:{enable:true}},callback:{<%--
                    ...
                    onCheck: function (event, treeId, treeNode) {
                        ...
                        setChildNodeNotCheck(ztreeObj,treeNode);
                    }
				}
			};
		...
		$(document).ready(function(){
			$.get("${ctx}${url}${fn:indexOf(url,'?')==-1?'?':'&'}&extId=${extId}&isAll=${isAll}&module=${module}&parentDealerId=${parentDealerId}&t="
                ...
                var parentAreaIds;
                $.ajax({
                    url: "${ctx}/fwzs/dealer/getSalesAreaIds",
                    type: "GET",
                    cache: false,
                    async : false,
                    data: {
                ...
                $(allNodes).each(function (i, item) {
                    $.each(parentAreaIds, function (k, checkedNode) {
                        if (item.id == checkedNode) {
                            tree.setChkDisabled(item, tree);
                        }
                    });
                });
				selectCheckNode();
        ...
        function setChildNodeNotCheck(tree, treeNode){
            var cNodes = treeNode.children;
            for (var i in cNodes) {
                cNodes[i].checked = false;
                tree.updateNode(cNodes[i]);
                setChildNodeNotCheck(tree, cNodes[i]);
            }
        }
