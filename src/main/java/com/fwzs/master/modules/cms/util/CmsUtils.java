package com.fwzs.master.modules.cms.util;
...
public class CmsUtils {
    ...
    public static List<Article> getArticleList(String siteId, String categoryId, int number, String param) {
        ...
        if (StringUtils.isNotBlank(param)) {
            ...
            Map map = JsonMapper.getInstance().fromJson("{" + param + "}", Map.class);
            if (new Integer(1).equals(map.get(ConfigConstant.POS_ID)) || new Integer(ConfigConstant.POS_ID_TWO).equals(map.get(ConfigConstant.POS_ID))) {
                article.setPosid(String.valueOf(map.get(ConfigConstant.POS_ID)));
            }
            if (new Integer(1).equals(map.get(ConfigConstant.IMAGE))) {
                article.setImage(Global.YES);
            }
            if (StringUtils.isNotBlank((String) map.get(ConfigConstant.ORDER_BY))) {
                page.setOrderBy((String) map.get(ConfigConstant.ORDER_BY));
            }
        }
    ...
    public static String getUrlDynamic(Category category) {
        if (StringUtils.isNotBlank(category.getHref())) {
            if (!category.getHref().contains(ConfigConstant.DELIMITER_DOUBLE_SLASH)) {
    ...
    public static String formatImageSrcToDb(String src) {
        ...
        if (src.startsWith(context.getContextPath() + ConfigConstant.USER_FILES)) {
